...
[![PyPI](https://img.shields.io/pypi/v/cognis-chargeguard.svg)](https://pypi.org/project/cognis-chargeguard/)
[![CI](https://github.com/cognis-digital/chargeguard/actions/workflows/ci.yml/badge.svg)](https://github.com/cognis-digital/chargeguard/actions)
...
CHARGEGUARD exists for one job — monitors dispute/chargeback feeds, flags fraud-rate threshold breaches (vamp/visa), and drafts representment evidence packets. — and does it without a SaaS bill or heavyweight setup.
...
```bash
chargeguard --version
chargeguard scan .                      # scan the current project
chargeguard scan . --format json
chargeguard scan . --fail-on high       # non-zero exit for CI gates
chargeguard mcp                         # expose as an MCP server (Cognis.Studio / Claude Desktop / Cursor)
```
...
Built in the spirit of **Stripe Radar / Chargebee disputes**, re-framed for the Cognis approach: single-purpose, self-hostable,
MCP-native, and unified with the rest of the Suite. Missing a credit? Open a PR.

## How it fits the Cognis Neural Suite

`chargeguard` is one of the **100+ tools** in the [Cognis Neural Suite](https://github.com/cognis-digital).
