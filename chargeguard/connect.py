"""Native cognis-connect emit for chargeguard — forward findings to any platform.

Maps chargeguard's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    chargeguard ... --format json | chargeguard-emit --to stix
    chargeguard-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('merchant_id', '') + ' ' + rec.get('description', '')
